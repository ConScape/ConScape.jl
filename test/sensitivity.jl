...
@testset "RSP sensitivity measure" begin
    ...
    @time new_x = solve((; sens=sensitivity_measures.sum_sens_cost), problems.no_grain.theta_one.ec, rast)
    heatmap(old_x)
    ...
    i = init(problems[1].theta_one.ec, rast)
    init(i, 1)
    ConScape.precalcualtions(i)
    ...
    @assert isapprox(OldConScape.store[].K, ConScape.store[].K)
    OldConScape.store[].diag
    ConScape.store[].diag
    ConScape.store[].diagC
    OldConScape.store[].diagC
    ...
    @test OldConScape.store[].X5 ≈ ConScape.store[].X5' atol=1e-10
    ...
    heatmap(new_sens.grain_two.theta_pointone.ec.sum_sens_likelihood; clims=(-3.0, 3.0))
    heatmap(old_sens.grain_two.theta_pointone.ec.sensitivity["C"]; clims=(-2.0, 0.1))
    heatmap(new_sens.grain_two.theta_pointone.ec.sum_sens_cost; clims=(-2.0, 0.1, ))
    ...
    @testset "square Z, no coarse graining" begin
        @testset "theta $theta" for (old_by_theta, new_by_theta, theta) in zip(old_sens.no_grain, new_sens.no_grain, thetas)
            @testset "$pm" for (old, new, pm) in zip(old_by_theta, new_by_theta, proximity_measures)
                @test isnanorapprox(old.sensitivity["C"],        new.sum_sens_cost; atol=1e-12)
                @test isnanorapprox(old.sensitivity["A"],        new.sum_sens_likelihood; atol=1e-7)
                @test isnanorapprox(old.sensitivity["C&A=f(C)"], new.sum_sens_likelihoodtocost; atol=1e-11)
                @test isnanorapprox(old.sensitivity["A&C=f(A)"], new.sum_sens_costtolikelihood; atol=1e-7)
                # @test isnanorapprox(old.elasticity["C"],         new.sum_elast_cost; atol=1e-11)
                # @test isnanorapprox(old.elasticity["A"],         new.sum_elast_likelihood; atol=1e-12)
                # @test isnanorapprox(old.elasticity["C&A=f(C)"],  new.sum_elast_likelihoodtocost; atol=1e-11)
                # @test isnanorapprox(old.elasticity["A&C=f(A)"],  new.sum_elast_costtolikelihood; atol=1e-11)
                # @test isnanorapprox(old.elasticity["Q"],         new.sum_elast_quality; atol=1e-13)
                # @test isnanorapprox(old.sensitivity["Q"],        new.sum_sens_quality; atol=1e-13)
            end
        end
    end
    ...
    @testset "non-square, coarse graining of 2" begin
        @testset "theta $theta" for (old_by_theta, new_by_theta, theta) in zip(old_sens.grain_two, new_sens.grain_two, thetas)
            @testset "$pm" for (old, new, pm) in zip(old_by_theta, new_by_theta, proximity_measures)
                # OldConScape cant do non-square Q
                @test isnanorapprox(old.sensitivity["C"],        new.sum_sens_cost; atol=1e-0)
                ...
                @test isnanorapprox(old.elasticity["C"],         new.sum_elast_cost; atol=1e-0)
