...
@testset "wall full" begin
    ...
    @testset "FunctionalHabitat" begin
        ...
        problem = ConScapeProblem(FunctionalHabitat(), RSP(ExpectedCost(); distance_transformation=ExpMinus(), kw...))
        @test solve(problem, g)[28:30, 58:60]' ≈ [
        ...
        problem = ConScapeProblem(FunctionalHabitat(), RSP(FreeEnergyDistance(); distance_transformation=ExpMinus(), kw...))
        @test solve(problem, g)[28:30,58:60] ≈ [
                 93.0825   140.907    362.669
                 41.1656    63.2089   159.685
                 3.65643    4.04458    4.23555] rtol=1e-3

        problem = ConScapeProblem(FunctionalHabitat(), RSP(SurvivalProbability(); kw...))
        @test solve(problem, g)[28:30,58:60] ≈ [
                 74141.1   72293.9    72294.7
                 27854.8   27066.5    26995.2
                 1151.38    765.195    391.131] rtol=1e-3

        problem = ConScapeProblem(FunctionalHabitat(), RSP(PowerMeanProximity(); kw...))
        @test solve(problem, g)[28:30,58:60] ≈ [
                 93.0825   140.907    362.669
                 41.1656    63.2089   159.685
                 3.65643    4.04458    4.23555] rtol=1e-3
    ...
    @testset "mean_lc_kl_divergence" begin
        @test_broken solve(ConScapeProblem(MeanKullbackLeiblerDivergence(), LCP()), g)[] ≈ 1.0667623231698838e14
    end

    @testset "eigmax for each proximity_measure" begin
        rsp = RSP(ExpectedCost(); theta=θ, distance_transformation=ExpMinus())
        M = solve(ConScapeProblem(LandscapeMatrix(), rsp), g)[1]
    ...
    @testset "Coarse graining: merging pixels to landmarks" begin
        ...
        @testset "eigmax for each proximity_measure" begin
            (vˡ, λ, vʳ) = solve(ConScapeProblem(EigMax(), RSP(ExpectedCost(); theta=θ)), g_coarse)[1]
            @test λ[] ≈ 2.7249231390873615e7
            (vˡ, λ, vʳ) = solve(ConScapeProblem(EigMax(), RSP(FreeEnergyDistance(); theta=θ)), g_coarse)[1]
            @test λ[] ≈ 2.7217089009360086e7
            (vˡ, λ, vʳ) = solve(ConScapeProblem(EigMax(), RSP(FreeEnergyDistance(); theta=θ)), g_coarse)[1]
            @test λ[] ≈ 2.7217089009360246e7
            (vˡ, λ, vʳ) = solve(ConScapeProblem(EigMax(), RSP(SurvivalProbability(); theta=θ)), g_coarse)[1]
        ...
        @testset "FunctionalHabitat" begin
            @testset "RSP ExpectedCost" begin
                ...
                fh_rsp = solve(ConScapeProblem(FunctionalHabitat(), rsp), g_coarse)
                rsp = RSP(ExpectedCost(); distance_transformation=ExpMinus(), theta=θ)
                fh_g = solve(ConScapeProblem(FunctionalHabitat(), rsp), g_coarse)
                rsp = RSP(ExpectedCost(); distance_transformation=ExpMinus(), theta=θ, approx=true)
                fh_g_approx = solve(ConScapeProblem(FunctionalHabitat(), rsp), g_coarse)
            ...
            @testset "LeastCostPath " begin
                ...
                fh_rsp_lc = solve(ConScapeProblem(FunctionalHabitat(), lc), g_coarse)
                fh_g_lc = solve(ConScapeProblem(FunctionalHabitat(), lc), g_coarse)
...
@testset "wall_landmark1" begin
    ...
    @testset "Landmark approach" begin
        @test solve(ConScapeProblem(Betweenness(QualityWeighted()), rsp), g)[9:11, 30:32] ≈
...
@testset "wall_landmark2" begin
    ...
    @testset "Landmark approach" begin
        @test solve(ConScapeProblem(Betweenness(QualityAndProximityWeighted()), rsp), g)[9:11, 30:32] ≈
