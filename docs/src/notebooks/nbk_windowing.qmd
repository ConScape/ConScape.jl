...
all required `Measure`s should be included in a single `ConScape.Problem`.
...
```{julia}
batch_problem = BatchProblem(windowed_problem; datapath=".", centersize=10, buffer)
```
...
```{julia}
results = RasterStack("dest_file.tif")
Rasters.rplot(results)
```
